...
from coffee_machine_client import get_dict_of_available_beverages, \
    order_n, get_stats, stop_server


class Window:
    """A class that contains widgets of tkinter window and methods
    used to interact with those widgets."""
    ...
    def __init__(self, main):
        ...
        self.refresh_b = tk.Button(
            master=self.main, text="Refresh data",
            fg="green", font=("Helvetica", 16), command=self.refresh)
        self.refresh_b.place(width=150, height=50, x=25, y=10)

        self.stats_b = tk.Button(
            master=self.main, text="Statistics", fg="black",
            font=("Helvetica", 16), command=self.stats)
        self.stats_b.place(width=150, height=50, x=self.WIDTH - 175,
                           y=10)

        self.state_label = tk.Label(master=self.main, text="",
                                    font=("Helvetica", 14))
    ...
    def order(self, i, event):
        ...
        try:
            ...
            message = "Connection error. Sorry, try again later."
    ...
    def refresh(self):
        """Gets dictionary of available beverages,
        then creates and places a button for each."""
        ...
        for i, bev in bevs.items():
            b = tk.Button(master=self.main, text=str(bev), fg="white",
                          bg="#b5651d", font=("Helvetica", 10))
            b.bind("<Button-1>",
                   lambda event, bev_i=i: self.order(bev_i, event))

            b.place(width=b_width, height=b_height,
                    x=start_width + (b_width + 10) * (count % 3),
                    y=start_height + b_height * (count // 3))
